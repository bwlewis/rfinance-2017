# This is an early, experimental prototype. The f option is not working yet,
# don't use it (that is, this version is limited to the exponential function).

topm <-
function(A,
         directed=TRUE,
         type=c("centrality","communicability",
...
{
# Note, need to dispatch to an appropriate undirected algorithm here...
# For now, we only support methods for directed problems.
  ...
  type <- match.arg(type)
  if(type != "centrality") stop("Only centralities supported just now...")
  ...
  while(TRUE)
  {
    ...
    if(N %% q == 0)
    {
# We need more singular vectors for strong convergence. Restart.
